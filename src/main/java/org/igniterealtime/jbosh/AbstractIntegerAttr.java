...
/**
 * Abstract base class for attribute implementations based on {@code Integer} types.  Additional support for parsing of integer
 * values from their {@code String} representations as well as callback handling of value validity checks are also provided.
 */
abstract class AbstractIntegerAttr extends AbstractAttr<Integer> {
    ...
    /**
     * Utility method intended to be called by concrete implementation classes from within the {@code check()} method when the
     * concrete class needs to ensure that the integer value does not drop below the specified minimum value.
     *
     * @param minVal minimum value to allow
     * @throws BOSHException if the integer value is below the specific minimum
     */
    ...
    /**
     * Returns the native {@code int} value of the underlying {@code Integer}. Will throw {@code NullPointerException} if the
     * underlying integer was {@code null}.
     *
